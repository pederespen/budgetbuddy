...
/**
 * Get suggested category based on common Norwegian merchants
 * This is just a helper - not mandatory
 */
export function getSuggestedCategoryName(
  ...
  if (isIncome) {
    if (pattern.includes("lønn")) return "Lønn";
    return "Annen Inntekt";
  }

  // Just some basic suggestions based on very common patterns
  const p = pattern.toLowerCase();

  if (
    ["kiwi", "rema", "meny", "coop", "bunnpris", "extra", "joker"].some((s) =>
      p.includes(s)
    )
  ) {
    return "Mat og Dagligvarer";
  }
  if (
    ["burger", "mcdonald", "cafe", "restaurant", "foodora", "wolt"].some((s) =>
      p.includes(s)
    )
  ) {
    return "Restaurant";
  ...
  if (
    ["ruter", "flytoget", "vy", "norwegian", "flybussen"].some((s) =>
      p.includes(s)
    )
  ) {
    return "Transport";
